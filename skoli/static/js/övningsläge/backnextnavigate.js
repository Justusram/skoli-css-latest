...
const backBtn = document.getElementById("tillbaka-btn")
backBtn.addEventListener('click', backOneQuestion)



const nextBtn = document.getElementById("nästa-btn")
nextBtn.addEventListener('click', nextQuestion)


function backOneQuestion(event) {


    //så att andra navbar länkar inte är klickade längre
    ...
    questionId = document.getElementById("question-id-div").textContent;


    var url = `http://127.0.0.1:8000/api/last-question/${questionId}`;
    ...
    .then(function(data){
        ...
        for (let choise of data.choises) {
            if (choise.correct_answer == true) {
                correctChoiseId = choise.id
                console.log(choise)
            }
        }


        //så att navbar länken till frågan visas som klickad
        const questionLinks = document.getElementsByClassName("question-link");
        for (const questionLink of questionLinks) {
            // Replace next question class
            if (questionLink.value == data.question.id) {
                questionLink.className = "question-link link-clicked";
            }
        }
        ...
        if (data.completed == true) {
            ...
            const correctRadio = document.querySelector(`input[type="radio"][name="answer"][value="${correctChoiseId}"]`);
        ...
        const tbxBtn = document.getElementById("tillbaka-btn")
        const nxtBtn = document.getElementById("nästa-btn")

        // Göm tbx btn om vi är på första frågan
        if (data.första_fråga == true) {
            tbxBtn.style.display = 'none';
        }
        else {
            tbxBtn.style.display = '';
        }
...
function nextQuestion(event) {
    ...
    var url = `http://127.0.0.1:8000/api/next-question/${questionId}`;
